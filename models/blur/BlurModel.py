...
class BlurRemoverModel:
    ...
    def preprocess_image(self, image_paths, desired_shape):
        ...
        for image_path in image_paths:
            ...
            if image is not None:
                ...
                image = cv2.resize(image, desired_shape[::-1])  # Resize to (width, height)
    ...
    def predict(self, image_path, desired_shape=(256, 256)):
        ...
        with torch.no_grad():
            image = self.preprocess_image([image_path], desired_shape)  # Pass as list
            image_tensor = torch.tensor(image).permute(0, 3, 1, 2).to(self.device)
            
            output_tensor = self.model(image_tensor)
            
            output_image = output_tensor.cpu().numpy().squeeze().transpose(1, 2, 0)  # Convert back to (H, W, C)
            output_image = np.clip(output_image * 255.0, 0, 255).astype(np.uint8)  # Scale back to [0, 255]
    ...
    def save_image(self, image, save_path):
        ...
        cv2.imwrite(save_path, image_bgr)  # Save the image using OpenCV
